...
public class LzhHandlerAdapter {
    ...
    LzhModelAndView handle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
        LzhHandlerMapping handlerMapping = (LzhHandlerMapping)handler;
        ...
        for (int i = 0; i < pa.length ; i ++) {
            for(Annotation a : pa[i]){
                if(a instanceof LzhRequestParam){
                    String paramName = ((LzhRequestParam) a).value();
        ...
        boolean isModelAndView = handlerMapping.getMethod().getReturnType() == LzhModelAndView.class;
        if(isModelAndView){
            return (LzhModelAndView) result;
        }
        return null;
    }
